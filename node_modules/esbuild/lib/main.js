...
function createChannel(streamIn) {
  ...
  let handleRequest = (id, request) => __async(this, null, function* () {
    try {
      ...
      sendResponse(id, {errors: [yield extractErrorMessageV8(e, streamIn, null)]});
    }
  });
  ...
  let handleIncomingPacket = (bytes) => {
    if (isFirstPacket) {
      ...
      if (binaryVersion !== "0.8.33") {
        throw new Error(`Cannot start service: Host version "${"0.8.33"}" does not match binary version ${JSON.stringify(binaryVersion)}`);
      }
      return;
    }
  ...
  let handlePlugins = (plugins, request, buildKey, details) => {
    if (streamIn.isSync)
          ...
          for (let id of request2.ids) {
            try {
              ...
              if (result != null) {
                ...
                if (errors != null)
                  response.errors = sanitizeMessages(errors, "errors", details);
                if (warnings != null)
                  response.warnings = sanitizeMessages(warnings, "warnings", details);
                break;
              }
            } catch (e) {
              return {id, errors: [yield extractErrorMessageV8(e, streamIn, details)]};
            }
          }
          return response;
        }
...
function extractErrorMessageV8(e, streamIn, details) {
...
function replaceDetailsInMessages(messages, details) {
  for (const message of messages) {
    message.detail = details.load(message.detail);
  }
  return messages;
}
function sanitizeMessages(messages, property, details) {
  let messagesClone = [];
  let index = 0;
  for (const message of messages) {
    let keys = {};
...
var startService = referenceCountedService(() => process.cwd(), (options) => {
  ...
  let child = child_process.spawn(command, args.concat(`--service=${"0.8.33"}`), {
    cwd: process.cwd(),
    windowsHide: true,
  ...
  return Promise.resolve({
    build: (options2) => {
      return refPromise(new Promise((resolve, reject) => {
        service.buildOrServe("build", null, options2, isTTY(), (err, res) => {
          if (err) {
            reject(err);
          } else {
            let oldRebuild = res.rebuild;
            if (oldRebuild) {
              let newRebuild = () => refPromise(oldRebuild().then((res2) => {
                res2.rebuild = newRebuild;
                return res2;
              }));
              newRebuild.dispose = oldRebuild.dispose;
              res.rebuild = newRebuild;
            }
            resolve(res);
          }
        });
    ...
    serve: (serveOptions, buildOptions) => {
      ...
      return refPromise(new Promise((resolve, reject) => service.buildOrServe("serve", serveOptions, buildOptions, isTTY(), (err, res) => {
        if (err) {
          reject(err);
        } else {
          refPromise(res.wait);
    ...
    transform: (input, options2) => {
      input += "";
      return refPromise(new Promise((resolve, reject) => service.transform("transform", input, options2 || {}, isTTY(), {
        readFile(tempFile, callback) {
          try {
...
var runServiceSync = (callback) => {
  ...
  callback(service);
  let stdout = child_process.execFileSync(command, args.concat(`--service=${"0.8.33"}`), {
    cwd: process.cwd(),
    windowsHide: true,
    input: stdin,
