...
var require_cjs = __commonJS((exports2) => {
  ...
  var SyncWalkerClass = class extends WalkerBase {
    constructor(walker) {
      super();
      this.enter = walker.enter;
      this.leave = walker.leave;
    }
    visit(node, parent, enter, leave, prop, index2) {
      if (node) {
        if (enter) {
          ...
          this.should_remove = false;
          this.replacement = null;
          enter.call(this.context, node, parent, prop, index2);
          if (this.replacement) {
        ...
        if (leave) {
          ...
          this.replacement = null;
          this.should_remove = false;
          leave.call(this.context, node, parent, prop, index2);
          if (this.replacement) {
            node = this.replacement;
            this.replace(parent, prop, index2, node);
          }
  ...
  var normalizePath = function(filename) {
    return filename.split(path7.win32.sep).join(path7.posix.sep);
  };
  function getMatcherString(id, resolutionBase) {
    if (resolutionBase === false || path7.isAbsolute(id) || id.startsWith("*")) {
      return id;
    }
    const basePath = normalizePath(path7.resolve(resolutionBase || "")).replace(/[-^$*+?.()|[\]{}]/g, "\\$&");
    return path7.posix.join(basePath, id);
  }
...
${baseIndent}` : ""}}`;
  ...
  exports2.attachScopes = attachScopes;
  exports2.createFilter = createFilter2;
  exports2.dataToEsm = dataToEsm;
  exports2.default = index;
  exports2.extractAssignedNames = extractAssignedNames;
  exports2.makeLegalIdentifier = makeLegalIdentifier;
...
var pluginutils = __toModule(require_cjs());

// src/utils/query.ts
var querystring = __toModule(require("querystring"));
function parseVueRequest(id) {
  const [filename, rawQuery] = id.split(`?`, 2);
  const query3 = querystring.default.parse(rawQuery);
  if (query3.vue != null) {
    query3.vue = true;
  }
  if (query3.src != null) {
  ...
  if (query3.index != null) {
    query3.index = Number(query3.index);
  }
  return {
    filename,
    query: query3
...
var hash_sum = __toModule(require_hash_sum());
var compiler_sfc = __toModule(require("@vue/compiler-sfc"));
var cache = new Map();
var prevCache = new Map();
function createDescriptor(filename, source, root, isProduction) {
  const {descriptor, errors} = compiler_sfc.parse(source, {
    filename,
    sourceMap: true
  });
  const normalizedPath = slash.default(path.default.normalize(path.default.relative(root, filename)));
  descriptor.id = hash_sum.default(normalizedPath + (isProduction ? source : ""));
...
// src/script.ts
var compiler_sfc3 = __toModule(require("@vue/compiler-sfc"));

// src/template.ts
var path3 = __toModule(require("path"));
var slash3 = __toModule(require_slash());
var compiler_sfc2 = __toModule(require("@vue/compiler-sfc"));
...
// src/template.ts
function transformTemplateAsModule(code, descriptor, options, pluginContext) {
...
function transformTemplateInMain(code, descriptor, options, pluginContext) {
  const result = compile(code, descriptor, options, pluginContext);
  return {
    ...result,
...
function compile(code, descriptor, options, pluginContext) {
  ...
  const result = compiler_sfc2.compileTemplate({
    ...resolveTemplateCompilerOptions(descriptor, options),
    source: code
  });
  if (result.errors.length) {
    result.errors.forEach((error3) => pluginContext.error(typeof error3 === "string" ? {id: filename, message: error3} : createRollupError(filename, error3)));
  }
...
function resolveTemplateCompilerOptions(descriptor, options) {
  ...
  if (options.devServer) {
    if (filename.startsWith(options.root)) {
      assetUrlOptions = {
        base: "/" + slash3.default(path3.default.relative(options.root, path3.default.dirname(filename)))
      };
    }
  } else {
    assetUrlOptions = {
  ...
  return {
    ...
    preprocessOptions,
    compilerOptions: {
      ...(_c = options.template) == null ? void 0 : _c.compilerOptions,
...
function getResolvedScript(descriptor, isServer = false) {
  return (isServer ? ssrCache : clientCache).get(descriptor);
}
function setResolvedScript(descriptor, script5, isServer = false) {
  ;
  (isServer ? ssrCache : clientCache).set(descriptor, script5);
}
function resolveScript(descriptor, options) {
  ...
  let resolved = null;
  resolved = compiler_sfc3.compileScript(descriptor, {
    ...options.script,
    id: descriptor.id,
    isProd: options.isProduction,
...
// src/main.ts
var querystring2 = __toModule(require("querystring"));
var path5 = __toModule(require("path"));
var compiler_sfc4 = __toModule(require("@vue/compiler-sfc"));

// src/handleHotUpdate.ts
var debug = __toModule(require_src());
var debug2 = debug.default("vite:hmr");
async function handleHotUpdate({
  ...
  if (updateType.length) {
    debug2(`[vue:update(${updateType.join("&")})] ${file}`);
  }
...
// src/main.ts
var source_map = __toModule(require_source_map());
async function transformMain(code, filename, options, pluginContext) {
  var _a;
  ...
  if (errors.length) {
    errors.forEach((error3) => pluginContext.error(createRollupError(filename, error3)));
    return null;
  }
  const hasScoped = descriptor.styles.some((s) => s.scoped);
  const {code: scriptCode, map} = await genScriptCode(descriptor, options, pluginContext);
  ...
  if (hasTemplateImport) {
    ;
    ({code: templateCode, map: templateMap} = await genTemplateCode(descriptor, options, pluginContext));
  }
  const renderReplace = hasTemplateImport ? ssr ? `_sfc_main.ssrRender = _sfc_ssrRender` : `_sfc_main.render = _sfc_render` : "";
  ...
  if (devServer && !isProduction) {
    ...
    output.push(`__VUE_HMR_RUNTIME__.createRecord(_sfc_main.__hmrId, _sfc_main)`);
  ...
  let resolvedMap = map;
  if (map && templateMap) {
    const generator = source_map.SourceMapGenerator.fromSourceMap(new source_map.SourceMapConsumer(map));
    const offset = ((_a = scriptCode.match(/\r?\n/g)) == null ? void 0 : _a.length) || 1;
    const templateMapConsumer = new source_map.SourceMapConsumer(templateMap);
...
async function genTemplateCode(descriptor, options, pluginContext) {
  const template4 = descriptor.template;
  if (!template4.lang && !template4.src) {
    return transformTemplateInMain(template4.content, descriptor, options, pluginContext);
  } else {
    if (template4.src) {
      await linkSrcToDescriptor(template4.src, descriptor, pluginContext);
    }
    const src = template4.src || descriptor.filename;
    const srcQuery = template4.src ? `&src` : ``;
    const attrsQuery = attrsToQuery(template4.attrs, "js", true);
    const query3 = `?vue&type=template${srcQuery}${attrsQuery}`;
    const request = JSON.stringify(src + query3);
...
async function genScriptCode(descriptor, options, pluginContext) {
  ...
  let map;
  const script5 = resolveScript(descriptor, options);
  if (script5) {
    if ((!script5.lang || script5.lang === "ts" && options.devServer) && !script5.src) {
      scriptCode = compiler_sfc4.rewriteDefault(script5.content, `_sfc_main`);
      map = script5.map;
      if (script5.lang === "ts") {
      ...
      if (script5.src) {
        await linkSrcToDescriptor(script5.src, descriptor, pluginContext);
      }
      const src = script5.src || descriptor.filename;
      const langFallback = script5.src && path5.default.extname(src).slice(1) || "js";
      const attrsQuery = attrsToQuery(script5.attrs, langFallback);
      const srcQuery = script5.src ? `&src` : ``;
      const query3 = `?vue&type=script${srcQuery}${attrsQuery}`;
      const request = JSON.stringify(src + query3);
...
async function genStyleCode(descriptor, pluginContext) {
  ...
  if (descriptor.styles.length) {
    for (let i = 0; i < descriptor.styles.length; i++) {
      const style2 = descriptor.styles[i];
      if (style2.src) {
        await linkSrcToDescriptor(style2.src, descriptor, pluginContext);
      }
      const src = style2.src || descriptor.filename;
      const attrsQuery = attrsToQuery(style2.attrs, "css");
      const srcQuery = style2.src ? `&src` : ``;
      const query3 = `?vue&type=style&index=${i}${srcQuery}`;
      const styleRequest = src + query3 + attrsQuery;
      if (style2.module) {
        if (!hasCSSModules) {
          stylesCode += `
const cssModules = _sfc_main.__cssModules = {}`;
        ...
        stylesCode += genCSSModulesCode(i, styleRequest, style2.module);
      } else {
        stylesCode += `
...
async function genCustomBlockCode(descriptor, pluginContext) {
  ...
  for (let index = 0; index < descriptor.customBlocks.length; index++) {
    ...
    const query3 = `?vue&type=${block.type}&index=${index}${srcQuery}${attrsQuery}`;
    const request = JSON.stringify(src + query3);
...
function attrsToQuery(attrs, langFallback, forceLangFallback = false) {
  ...
  for (const name in attrs) {
    ...
    if (!ignoreList.includes(name)) {
      query3 += `&${querystring2.default.escape(name)}${value ? `=${querystring2.default.escape(String(value))}` : ``}`;
    }
  }
  if (langFallback || attrs.lang) {
    query3 += `lang` in attrs ? forceLangFallback ? `&lang.${langFallback}` : `&lang.${attrs.lang}` : `&lang.${langFallback}`;
  }
  return query3;
}

// src/style.ts
var compiler_sfc5 = __toModule(require("@vue/compiler-sfc"));
async function transformStyle(code, descriptor, index, options, pluginContext) {
  const block = descriptor.styles[index];
  const result = await compiler_sfc5.compileStyleAsync({
    ...options.style,
...
function vuePlugin(rawOptions = {}) {
  ...
  const filter = pluginutils.createFilter(rawOptions.include || /\.vue$/, rawOptions.exclude);
  return {
    name: "vite:vue",
    handleHotUpdate(ctx) {
      if (!filter(ctx.file)) {
    ...
    load(id) {
      const {filename, query: query3} = parseVueRequest(id);
      if (query3.vue) {
        if (query3.src) {
        ...
        let block;
        if (query3.type === "script") {
          block = getResolvedScript(descriptor, options.ssr);
        } else if (query3.type === "template") {
          block = descriptor.template;
        } else if (query3.type === "style") {
          block = descriptor.styles[query3.index];
        } else if (query3.index != null) {
          block = descriptor.customBlocks[query3.index];
        }
        if (block) {
          return {
            code: block.content,
    ...
    transform(code, id) {
      ...
      if (!query3.vue) {
        ...
        const descriptor = getDescriptor(filename);
        if (query3.type === "template") {
          return transformTemplateAsModule(code, descriptor, options, this);
        } else if (query3.type === "style") {
          return transformStyle(code, descriptor, Number(query3.index), options, this);
        }
      }
    }
  };
